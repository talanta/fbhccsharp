...
namespace HackerCup._2012.R2
{
	...
	Let S be a sequence of N natural numbers. We can define an infinite sequence MS in 
	the following way: MS[k] = S[k mod N] + N * floor(k / N). Where k is a zero based index.

	For example if the sequence S is {2, 1, 3} then MS would be 
		{2, 1, 3, 5, 4, 6, 8, 7, 9, 11, 10, 12...}

	Now consider a subsequence of MS generated by picking two random indices a, b from 
	the range [0..R] inclusive, and taking all the elements between them, 
	that is: MS[min(a, b)..max(a, b)].
	...
	Input
	The first line of the input file contains an integer T. This is followed by T test cases, 
	each of which has two lines.
	...
	Output
	For each of the test cases numbered in order from 1 to T, output "Case #i: " followed by 
	the probability that the selected subsequence of MS has at least K distinct elements. 
	The probability should be expressed as a fraction p/q, where p and q represent the 
	numerator and denominator respectively and are relatively prime (that is they share no 
	...
	The third example uses the same sequence as the second example, but now we want to have 
	subsequences with at least 4 different numbers. All pairs of indices that have this 
	property are: (0,4), (0, 5), (1, 5), (4, 0), (5, 0), and (5, 1). Six out of thirty six 
	results in a probability of 1/6.
	*/

    public class SequenceSlicing : IExecutable
    {
