namespace HackerCup._2015.R1
{
    ...
    There are N employees working at Corpro Corp., and each of them has a manager,
    except for the CEO who has no manager (the CEO also buys a gift every year, but
    she donates it to charity). The employees each have a unique employee ID which
    is an integer from 1 to N. As you might expect, the CEO has the ID 1.

    If there exists a set of two or more employees {p1, ..., pk} such that,
    for all i < k, pi is the manager of pi+1, then we say that p1 is "responsible for" pk.
    There are never two employees who are responsible for each other. That would be
    a silly hierarchy indeed.

    There are N kinds of gifts available for purchase, and the ith kind of gift
    costs i dollars. That is, the prices of the different kinds of gifts
    are {$1, $2, $3, ... $N}. There are N copies of each gift available for purchase.
    ...
    buy a $2 gift instead. Note that it would be equally optimal for the CEO to buy
    a $2 gift, while receiving a $1 gift from her subordinate.
    ...
    Each hierarchy is made up of two lines. The first line contains the integer N.
    The second line contains N space-separated integers. The ith integer is the employee
    ID of the manager of employee i, with the exception that the first integer is
    always 0, denoting that the CEO has no manager.

    Output

    For the ith hierarchy, print a line containing "Case #i: " followed by the
    smallest amount of money the entire company would need to spend.
    ...
    */
    public class CorporateGifting : IExecutable
    {
